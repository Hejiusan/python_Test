...
def affine_forward(x, w, b):
    ...
    # 对于形状为(4,2)的数组，其形状不变，对于形状为(4,20,20)的数组，形状变为（4,20*20）
...
for j in range(10000):   #这里设置了训练的循环次数为10000
    ...
    probs /= np.sum(probs, axis=1, keepdims=True)        # Softmax算法实现(输出的正规化)
    # ③计算loss值
    N = Y.shape[0]                                       # 值为4
    ...
    # np.arange(N) 生成一个包含从 0 到 N-1 的数组，其中 N 是样本数。
    # probs[np.arange(N), t] 选择在每个样本中对应于真实标签 t 的概率
